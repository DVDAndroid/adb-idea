...
public class DeviceChooserDialog extends DialogWrapper {
	@NonNls
	private static final String SELECTED_SERIALS_PROPERTY = DeviceChooserDialog.class
			.getCanonicalName() + "-SELECTED_DEVICES";
	...
	public DeviceChooserDialog(@NotNull final AndroidFacet facet) {
		...
		final String serialsStr = properties
				.getValue(SELECTED_SERIALS_PROPERTY);
		if (serialsStr != null) {
	...
	private DeviceChooser buildPostZeroDotSixDeviceChooser(AndroidFacet facet) {
		return new DeviceChooser(true, getOKAction(), facet,
				facet.getConfiguration().getAndroidTarget(), null);
	}
	...
	protected void doOKAction() {
		...
		final PropertiesComponent properties = PropertiesComponent
				.getInstance(myProject);
		properties.setValue(SELECTED_SERIALS_PROPERTY,
				toString(myDeviceChooser.getSelectedDevices()));
