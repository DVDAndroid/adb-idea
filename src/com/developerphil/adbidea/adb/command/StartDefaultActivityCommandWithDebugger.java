...
public class StartDefaultActivityCommandWithDebugger implements Command {
	...
	public boolean run(final Project project, final IDevice device,
		...
		try {
			...
			if (!status) {
				...
				info(String.format("<b>%s</b> forced-stop on %s", packageName,
						device.getName()));
			}
	...
	private void runDebugger(String port, Project project,
		...
		final String configurationName = String
				.format("(ADB IDEA) Debugger (%s)", new Object[]{port});
		...
		descriptors = ExecutionHelper.findRunningConsoleByTitle(prvProject,
				new NotNullFunction() {
					@NotNull
					@Override
					public Object fun(Object title) {
						return Boolean.valueOf(
								configurationName.equals((String) title));
					}
				});
		...
		if (size > 0) {
			for (int i = 0; i < size; i++) {
				prvDescriptor = (RunContentDescriptor) descriptors.iterator()
						.next();
				...
				if ((processHandler != null) && (content != null)) {
					prvExecutor = DefaultDebugExecutor
							.getDebugExecutorInstance();
					if (processHandler.isProcessTerminated()) {
						SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								ExecutionManager.getInstance(prvProject)
										.getContentManager().removeRunContent(
												prvExecutor, prvDescriptor);
							}
						});
					}
				}
	...
	private boolean startDebugging(final IDevice device, final Project project,
		...
		try {
			...
			if ((bridge != null) && (canDdmsBeCorrupted)) {
				error(String.format(
						"ERROR: ddms can be corrupted, can't start debugger."));
				return false;
			}
			...
			if (client == null) {
				error(String.format(
						"ERROR: client == null, can't start debugger."));
				return false;
			}

			final String port = Integer
					.toString(client.getDebuggerListenPort());
			runDebugger(port, project, device);

			return true;
		} catch (Exception e) {
			error(String.format(
					"ERROR: Fatal Exception, unable to start debugger!"));
		}
