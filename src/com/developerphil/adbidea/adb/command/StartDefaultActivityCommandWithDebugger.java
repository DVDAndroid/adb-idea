...
public class StartDefaultActivityCommandWithDebugger implements Command {
		...
		for (Activity activity : application.getActivities()) {
			for (IntentFilter filter : activity.getIntentFilters()) {
				if (AndroidDomUtil.containsAction(filter, LAUNCH_ACTION_NAME)
						&& AndroidDomUtil.containsCategory(filter,
								LAUNCH_CATEGORY_NAME)) {
	...
	@Override
	public boolean run(final Project project, final IDevice device,
			final AndroidFacet facet, final String packageName) {
		...
		try {
			...
			device.executeShellCommand("am start -D -n " + component, receiver,
					5L, TimeUnit.MINUTES);

			boolean status = startDebugging(device, project, facet,
					packageName);
			if (!status) {
			...
			if (receiver.isSuccess()) {
				info(String.format("<b>%s</b> started on %s", packageName,
						device.getName()));
				return true;
			} else {
				error(String.format(
						"<b>%s</b> could not bet started on %s. \n\n<b>ADB Output:</b> \n%s",
						packageName, device.getName(), receiver.getMessage()));
			}
		} catch (Exception e) {
		...
		return ApplicationManager.getApplication()
				.runReadAction(new Computable<String>() {
                    ...
                    public String compute() {
                        return getDefaultLauncherActivityName(
                                facet.getManifest());
                    }
                });
	}
	...
	private void closeOldSessionAndRun(String port, Project project) {
		final String configurationName = String.format("Android Debugger (%s)",
				new Object[] { port });
		prvProject = project;
		Collection descriptors = ExecutionHelper
				.findRunningConsoleByTitle(project, new NotNullFunction() {
                    @NotNull
                    public Boolean fun(String title) {
                        Boolean status = Boolean
                                .valueOf(configurationName.equals(title));
                        if (status == null)
                            throw new IllegalStateException(String.format(
                                    "ERROR: parameters must not return null"));
                        return status;
                    }
		...
		if (descriptors.size() > 0) {
			RunContentDescriptor descriptor = (RunContentDescriptor) descriptors
					.iterator().next();
			ProcessHandler processHandler = descriptor.getProcessHandler();
			Content content = descriptor.getAttachedContent();

			if ((processHandler != null) && (content != null)) {
				Executor executor = DefaultDebugExecutor
		...
		RemoteConfigurationType remoteConfigurationType = RemoteConfigurationType
				.getInstance();
		...
		prvSettings = RunManager.getInstance(project)
				.createRunConfiguration(configurationName, factory);

		RemoteConfiguration configuration = (RemoteConfiguration) prvSettings
				.getConfiguration();
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				ProgramRunnerUtil.executeConfiguration(prvProject, prvSettings,
						DefaultDebugExecutor.getDebugExecutorInstance());
			}
		});
	}

	private boolean startDebugging(final IDevice device, final Project project,
			final AndroidFacet facet, final String packageName) {
		if (device == null)
			throw new IllegalArgumentException(
					String.format("ERROR: startDebugging(): device == null"));
		prvPackageName = packageName;
		info(String.format("Target device: " + device.getName(),
		...
		try {
			...
			if (device.hasClients()) {
				...
				for (int i = 0; i < clients.length; i++) {
					if (packageName.equalsIgnoreCase(clients[i].getClientData()
			...
			ApplicationManager.getApplication().invokeLater(new Runnable() {
				public void run() {
					androidToolWindow = ToolWindowManager.getInstance(project)
							.getToolWindow(
                                    AndroidToolWindowFactory.TOOL_WINDOW_ID);
					androidToolWindow.activate(new Runnable() {
						public void run() {
							int count = androidToolWindow.getContentManager()
									.getContentCount();
							for (int i = 0; i < count; i++) {
								Content content = androidToolWindow
										.getContentManager().getContent(i);
								DevicePanel devicePanel = content == null ? null
										: (DevicePanel) content.getUserData(
												AndroidToolWindowFactory.DEVICES_PANEL_KEY);
								AndroidLogcatView logcatView = content == null
										? null
										: (AndroidLogcatView) content
												.getUserData(
								...
								if (devicePanel != null) {
									...
									logcatView.createAndSelectFilterByPackage(
											prvPackageName);
									break;
								}
							}
